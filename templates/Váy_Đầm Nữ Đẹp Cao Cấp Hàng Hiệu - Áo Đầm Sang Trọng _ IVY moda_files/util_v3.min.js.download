...
else this[fields[i].users_field]={id:-1,description:"LBL_UNKNOWN",user_field_id:fields[i].user_field_id,translate:!0}
...
return pattern.test(emailAddress)}function isValidPhoneNumber(phone){return/^0[0-9]{8,12}$/.test(phone)&&phone.length>=3&&phone.length<=20}function getParameterByName(key,queryString){name=name.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]")
