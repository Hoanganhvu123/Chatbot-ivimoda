...
message.leftSlider-=165,message.leftSlider-165<-length&&(message.leftSlider=-length+165)},$scope.doInitDomain=function(domain,parentUsername){DomainService.getChat(domain,parentUsername).then(function(data){if(null==data||void 0==data)return void($scope.isBanned=!0)
if(data.data&&1==data.data.is_banned)return $scope.isBanned=!0,void sendTrackingEvent("visitor_ip_banned",{ip:data.data?data.data.ip:null})
var addition_setting=data.data.addition_setting?angular.fromJson(data.data.addition_setting):null
